"""
Módulo de reservas y demostración. Define la clase Reserva (con confirmación,
cancelación, procesamiento) y la función simular_operaciones() que ejecuta
10 casos válidos e inválidos para probar el manejo de excepciones.
"""

from base import Entity, ReservationError, InvalidDataError, logger
...
class Reserva(Entity):
    """Gestiona una reserva que asocia un cliente, un servicio y una duración."""
    ...
    def __init__(self, cliente: Cliente, servicio: Servicio, unidades: float) -> None:
        ...
        logger.info(f"Reserva creada: cliente={cliente.nombre}, servicio={servicio.nombre}, unidades={unidades}")

    def calcular_costo(self) -> float:
        """Calcula el costo delegando en el servicio."""
        try:
    ...
    def procesar(self) -> float:
        """Procesa la reserva: calcula costo y confirma. Usa try/except/else/finally."""
        try:
            logger.debug(f"Procesando reserva {self.id} para {self.cliente.nombre}.")
...
def simular_operaciones() -> None:
    """Ejecuta 10 operaciones (válidas e inválidas) demostrando robustez."""
    ...
    # 5. Equipo válido
    try:
        eq1 = Equipo("Proyector", "Proyector HD", tarifa_hora=15.0, tipo_equipo="proyector", deposito=50.0)
        servicios.append(eq1)
        operaciones.append("Equipo válido creado")
    except InvalidDataError as e:
        operaciones.append(f"Fallo equipo: {e}")

    # 6. Asesoría con costo base negativo
    try:
        ases1 = Asesoria("Consultoría", "Costo negativo", costo_base=-100.0)  # type: ignore
    ...
    # 9. Reserva con unidades negativas
    try:
        if clientes and servicios:
